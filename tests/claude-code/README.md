...
### Run specific test:
```bash
./run-skill-tests.sh --test test-worktree-path-policy.sh
```
...
Checks the using-git-worktrees path policy.
...
```bash
./run-skill-tests.sh --verbose --test test-worktree-path-policy.sh
```
