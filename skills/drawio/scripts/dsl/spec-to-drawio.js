...
export function generateNodeStyle(node, theme) {
  ...
  const rounded = node.style?.rounded !== undefined ? node.style.rounded : (nodeTheme.rounded !== undefined ? nodeTheme.rounded : defaultTheme.rounded)
  const arcSize = node.style?.arcSize !== undefined ? node.style.arcSize : (nodeTheme.arcSize !== undefined ? nodeTheme.arcSize : defaultTheme.arcSize)
...
export function generateConnectorStyle(edge, theme, routing = 'orthogonal') {
  ...
  const endSize = edge.style?.endSize ?? connectorTheme.endSize
  const startSize = edge.style?.startSize ?? connectorTheme.startSize
