 ...
 * @param {object[]} cells - all cells to scan for custom arrow sizes
 * @returns {string}
 */
function buildMarkerDefs(cells) {
  const markers = []
  const markerKeys = new Set()

  // Add default markers to ensure backward compatibility/fallbacks
  markerKeys.add("block:6")
  markerKeys.add("open:6")
  markerKeys.add("classic:6")
  markerKeys.add("diamond:8")
  ...
  if (cells) {
    for (const cell of cells) {
      ...
      if (endArrow && endArrow !== 'none') {
        const endSize = style.get('endSize') || (endArrow === 'diamond' ? '8' : '6')
        markerKeys.add(`${endArrow}:${endSize}`)
      }
      const startArrow = style.get('startArrow')
      if (startArrow && startArrow !== 'none') {
        const startSize = style.get('startSize') || (startArrow === 'diamond' ? '8' : '6')
        markerKeys.add(`${startArrow}:${startSize}`)
      }
    }
  }
  ...
  for (const key of markerKeys) {
    ...
    const size = Number(sizeStr) || 6
    if (type === 'block') {
      markers.push(
        `  <marker id="arrow-block-${size}" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="${size}" markerHeight="${size}" orient="auto-start-reverse">`,
        '    <path d="M 0 0 L 10 5 L 0 10 Z" fill="currentColor"/>',
        '  </marker>'
      )
    } else if (type === 'open') {
      markers.push(
        `  <marker id="arrow-open-${size}" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="${size}" markerHeight="${size}" orient="auto-start-reverse">`,
        '    <path d="M 0 0 L 10 5 L 0 10" fill="none" stroke="currentColor" stroke-width="1.5"/>',
        '  </marker>'
      )
    } else if (type === 'classic') {
      markers.push(
        `  <marker id="arrow-classic-${size}" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="${size}" markerHeight="${size}" orient="auto-start-reverse">`,
        '    <path d="M 0 0 L 10 5 L 0 10 L 3 5 Z" fill="currentColor"/>',
        '  </marker>'
      )
    } else if (type === 'diamond') {
      markers.push(
        `  <marker id="arrow-diamond-${size}" viewBox="0 0 12 12" refX="12" refY="6" markerWidth="${size}" markerHeight="${size}" orient="auto-start-reverse">`,
  ...
  // Include default un-sized markers as fallbacks
  markers.push(
    '  <marker id="arrow-block" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">',
    '    <path d="M 0 0 L 10 5 L 0 10 Z" fill="currentColor"/>',
    '  </marker>',
    '  <marker id="arrow-open" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">',
    '    <path d="M 0 0 L 10 5 L 0 10" fill="none" stroke="currentColor" stroke-width="1.5"/>',
    '  </marker>',
    '  <marker id="arrow-classic" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">',
    '    <path d="M 0 0 L 10 5 L 0 10 L 3 5 Z" fill="currentColor"/>',
    '  </marker>',
    '  <marker id="arrow-diamond" viewBox="0 0 12 12" refX="12" refY="6" markerWidth="8" markerHeight="8" orient="auto-start-reverse">',
    '    <path d="M 0 6 L 6 0 L 12 6 L 6 12 Z" fill="currentColor"/>',
    '  </marker>'
  )
 ...
 */
function markerRef(arrowType, position, size) {
  if (!arrowType || arrowType === 'none') return ''
  const baseType = ARROW_TYPES.includes(arrowType) ? arrowType : 'block'
  const id = size ? `arrow-${baseType}-${size}` : `arrow-${baseType}`
  ...
  if (label) {
    ...
    if (lines.length > 1) {
      ...
      lines.forEach((line, index) => {
        ...
        tspanParts.push(`<tspan x="${textX}" y="${lineY}">${escapeXml(line)}</tspan>`)
      })
      
      parts.push(
        `<text text-anchor="${textAnchor}" dominant-baseline="${dominantBaseline}" ` +
...
/**
 * Find the intersection point of a line segment pOuter -> pInner with the boundary of a box.
 * @param {{x: number, y: number}} pOuter
 * @param {{x: number, y: number}} pInner
 * @param {{x: number, y: number, width: number, height: number}} box
 * @returns {{x: number, y: number}}
 */
function getBoxIntersection(pOuter, pInner, box) {
  if (!box) return pInner
  const xMin = box.x
  const xMax = box.x + box.width
  const yMin = box.y
  const yMax = box.y + box.height

  const dx = pInner.x - pOuter.x
  const dy = pInner.y - pOuter.y

  const candidates = []

  if (dx !== 0) {
    // Left edge (x = xMin)
    let t = (xMin - pOuter.x) / dx
    if (t >= 0 && t <= 1) {
      let yInter = pOuter.y + t * dy
      if (yInter >= yMin && yInter <= yMax) {
        candidates.push({ x: xMin, y: yInter, t })
      }
    }
    // Right edge (x = xMax)
    t = (xMax - pOuter.x) / dx
    if (t >= 0 && t <= 1) {
      let yInter = pOuter.y + t * dy
      if (yInter >= yMin && yInter <= yMax) {
        candidates.push({ x: xMax, y: yInter, t })
      }
    }
  }
  ...
  if (dy !== 0) {
    // Top edge (y = yMin)
    let t = (yMin - pOuter.y) / dy
    if (t >= 0 && t <= 1) {
      let xInter = pOuter.x + t * dx
      if (xInter >= xMin && xInter <= xMax) {
        candidates.push({ x: xInter, y: yMin, t })
      }
    }
    // Bottom edge (y = yMax)
    t = (yMax - pOuter.y) / dy
    if (t >= 0 && t <= 1) {
      let xInter = pOuter.x + t * dx
      if (xInter >= xMin && xInter <= xMax) {
        candidates.push({ x: xInter, y: yMax, t })
      }
    }
  }

  if (candidates.length > 0) {
    // Sort by t ascending (closest to pOuter)
...
function renderEdge(cell, style, cellMap, suppressLabel = false) {
  ...
  let sourceClipNeeded = false
  if (sourceCell) {
    const geo = sourceCell.geometry || { x: 0, y: 0, width: 120, height: 60 }
    sourceCenter = { x: geo.x + geo.width / 2, y: geo.y + geo.height / 2 }
    const exitX = style.get('exitX')
    const exitY = style.get('exitY')
    if (exitX !== undefined && exitY !== undefined) {
      x1 = geo.x + geo.width * Number(exitX)
      y1 = geo.y + geo.height * Number(exitY)
    } else {
      x1 = sourceCenter.x
      y1 = sourceCenter.y
      sourceClipNeeded = true
    }
  }

  let x2 = 100, y2 = 100
  let targetCenter = null
  let targetClipNeeded = false
  if (targetCell) {
    const geo = targetCell.geometry || { x: 0, y: 0, width: 120, height: 60 }
    targetCenter = { x: geo.x + geo.width / 2, y: geo.y + geo.height / 2 }
    const entryX = style.get('entryX')
    const entryY = style.get('entryY')
    if (entryX !== undefined && entryY !== undefined) {
      x2 = geo.x + geo.width * Number(entryX)
      y2 = geo.y + geo.height * Number(entryY)
    } else {
      x2 = targetCenter.x
      y2 = targetCenter.y
      targetClipNeeded = true
    }
  }
  ...
  if (sourceClipNeeded && sourceCell && sourceCell.geometry) {
    const nextPt = points.length > 0 ? points[0] : targetCenter
    if (nextPt) {
      const clipped = getBoxIntersection(nextPt, sourceCenter, sourceCell.geometry)
      x1 = clipped.x
      y1 = clipped.y
    }
  }

  if (targetClipNeeded && targetCell && targetCell.geometry) {
    const prevPt = points.length > 0 ? points[points.length - 1] : sourceCenter
    if (prevPt) {
      const clipped = getBoxIntersection(prevPt, targetCenter, targetCell.geometry)
      x2 = clipped.x
      y2 = clipped.y
    }
  }
  ...
  const endSize = style.get('endSize') || (endArrow === 'diamond' ? '8' : '6')
  const startSize = style.get('startSize') || (startArrow === 'diamond' ? '8' : '6')
  const endRef = markerRef(endArrow, 'end', endSize)
  const startRef = markerRef(startArrow, 'start', startSize)
  const colorStyle = ` style="color: ${strokeColor}"`

  const pathPoints = [{ x: x1, y: y1 }, ...points, { x: x2, y: y2 }]
  const uniquePoints = []
  ...
  if (uniquePoints.length >= 2) {
    if (points.length === 0) {
      parts.push(
        `<line x1="${uniquePoints[0].x}" y1="${uniquePoints[0].y}" x2="${uniquePoints[1].x}" y2="${uniquePoints[1].y}" ` +
          `stroke="${strokeColor}" stroke-width="${strokeWidth}"${dashAttr}` +
          `${endRef}${startRef}${colorStyle} fill="none"/>`
      )
    } else {
      const d = uniquePoints.map((pt, i) => `${i === 0 ? 'M' : 'L'} ${pt.x} ${pt.y}`).join(' ')
      parts.push(
        `<path d="${d}" ` +
          `stroke="${strokeColor}" stroke-width="${strokeWidth}"${dashAttr}` +
          `${endRef}${startRef}${colorStyle} fill="none"/>`
...
export function drawioToSvg(xmlString) {
  ...
  // Defs (arrow markers)
  svgParts.push(buildMarkerDefs(cells))
