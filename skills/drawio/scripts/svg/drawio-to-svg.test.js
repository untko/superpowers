...
<mxGraphModel background="#E0F2FE">
  ...
  it('should render edge with waypoints as <path> element', () => {
    const xml = `
<mxGraphModel>
  <root>
    <mxCell id="0"/>
    ...
    const svg = drawioToSvg(xml)
    assert.ok(svg.includes('<path d="M'), 'Edge with waypoints should render as <path>')
    assert.ok(svg.includes('100 150'), 'Path should include waypoint X/Y')
    assert.ok(svg.includes('200 150'), 'Path should include waypoint X/Y')
  })

  it('should clip edge endpoints to cell boundaries when no exitX/exitY or entryX/entryY specified', () => {
    const xml = `
<mxGraphModel>
  <root>
    <mxCell id="0"/>
    ...
    const svg = drawioToSvg(xml)
    assert.ok(svg.includes('x1="150"'), 'Edge start should be clipped to source boundary')
    assert.ok(svg.includes('y1="75"'), 'Edge start should align vertically')
    assert.ok(svg.includes('x2="250"'), 'Edge end should be clipped to target boundary')
    assert.ok(svg.includes('y2="75"'), 'Edge end should align vertically')
  })
